...
public class YoutubeLinkField : SmartUIElement
{
    ...
    public override void SafeDoubleClick(UIMouseEvent evt)
    {
        ...
        ExtensionFilter[] filters = [ new ExtensionFilter("MP3 files", "mp3") ];
        ...
        if (files.Length > max)
        {
            Main.NewText(LocalizationHelper.GetGUIText("AsyncMP3Downloader.TooManyFiles", files.Length, max), MP3Player.Pink);
        ...
        foreach (string file in files)
        {
            ...
            // Converts the target MP3 file to the standard format used by UltraSonic and then outputs it to the cache directory.
            FFmpegImport(file, originalFileName, titleFile, songFile);
        }
    }

    private static void FFmpegImport(string file, string originalFileName, string titleFile, string songFile)
    {
        Task.Run(() =>
        {
            string args = $"-i \"{file}\" -vn -f mp3 -ab 320k \"{songFile}\"";

            Process process = FFmpeg.Run(MP3Player.CachePath, args);

            // Wait until ffmpeg conversion is finished before repopulating song list.
            ...
            // After download is done, this will switch panels and repopulate the list.
