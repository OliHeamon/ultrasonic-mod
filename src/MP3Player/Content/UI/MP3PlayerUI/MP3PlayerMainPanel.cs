...
public class MP3PlayerMainPanel : SmartUIElement
{
    ...
    public override void OnInitialize()
    {
        ...
        {
            // If the text field is empty, the "+" button will instead paste the clipboard into the box.
            if (string.IsNullOrEmpty(linkField.CurrentValue))
            {
                // We are interfacing with SDL directly for clipboard text as tML's implementation does not work on linux.
                linkField.CurrentValue = SDL2.SDL.SDL_GetClipboardText();
                return;
            }
